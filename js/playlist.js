...
for (var i = 0; i < playlist.length; i++) {
    ...
    var spanArtists = document.createElement("span");

    h3.innerText = playlist[i].name;
    spanAlbum.innerText = playlist[i].album.name;
    spanAlbum.innerText = playlist[i].artists;


    spanAlbum.classList.add("album");
    spanArtists.classList.add("Artists");

    li.setAttribute("data-id", i); //<li> </li>
    li.appendChild(h3);
    li.appendChild(spanAlbum);

    li.appendChild(spanArtists);


    playlistUl.appendChild(li);      /* como copiasse uma novo ul em cada volta. cada volta pega uma child*/
...
var lis = document.querySelectorAll("#playlist li");
    for(var i = 0; i< lis.length; i++){
        ...
        if(titulo.innerText.toLowerCase().search(this.value.toLowerCase()) > -1){
            ...
            console.log("NAO tem uma musica com esse nome");
